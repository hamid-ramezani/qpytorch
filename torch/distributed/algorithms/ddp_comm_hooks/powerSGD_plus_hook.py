...
class PowerSGD_plus_State(object):
    r"""
...
def powerSGD_plus_hook(
    ...
    Args:
        state (PowerSGD_plus_State): State information to configure the compression rate and support error feedback, warm start, etc.
...
def batched_powerSGD_plus_hook(
    state: PowerSGD_plus_State, bucket: dist.GradBucket
) -> torch.futures.Future:
    r"""
    ...
    Therefore, it is **faster** than :meth:`powerSGD_plus_hook`,
    ...
    .. warning ::
        ...
        Therefore, the user should always consider :meth:`powerSGD_plus_hook` first,
    ...
    Example::
        >>> state = PowerSGD_plus_State(process_group=process_group, matrix_approximation_rank=[1]*20)
        >>> ddp_model.register_comm_hook(state, batched_powerSGD_plus_hook)
    """  # noqa: B950
    process_group = state.process_group
    group_to_use = process_group if process_group is not None else dist.group.WORLD
    world_size = group_to_use.size()
